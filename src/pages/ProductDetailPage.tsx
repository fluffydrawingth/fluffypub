...
export default function ProductDetailPage({ slug }: { slug: string }) {
  ...
  if (!product) return (
    <div style={{textAlign:'center',padding:'80px 24px',fontFamily:theme.fontFamily}}>
      ...
      <button onClick={()=>navigate('/products')} style={{background:theme.primaryColor,color:'white',border:'none',cursor:'pointer',padding:'12px 28px',borderRadius:24,marginTop:16,fontSize:15,fontWeight:700,fontFamily:theme.fontFamily}}>{tRaw('กลับไปช้อป','Back to Shop')}</button>
    </div>
  );
  ...
  // THB price only — variant price_thb takes priority, then product price_thb
  const displayTHB: number = selectedVariant
    ? (Number(selectedVariant.price_thb) > 0 ? Number(selectedVariant.price_thb) : Number(product.price_thb) || 0)
    : Number(product.price_thb) || 0;

  const originalTHB = Number(product.original_price_thb || product.compare_price_thb || 0);
  const discountPct = (originalTHB > displayTHB && displayTHB > 0)
    ? Math.round((1 - displayTHB / originalTHB) * 100) : null;
  ...
  const handleAddToCart = () => {
    if (hasVariants && !selectedVariant) {
      setVariantError(tRaw('กรุณาเลือกรูปแบบก่อน', 'Please select an option.'));
      return;
    }
    setVariantError('');

    // Product type: never override with "digital" unless actually digital
    const itemType = isPhysical && !isDigital ? 'physical'
      : isDigital && !isPhysical ? 'digital'
      : product.type || 'physical';

    add({
      id: product.id,
      title,
      price_thb: displayTHB,
      image: product.image,
      ...
      <div className="pd-outer" style={{maxWidth:1200,margin:'0 auto',padding:'0 16px 48px'}}>
        <div className="pd-grid" style={{background:'white',borderRadius:20,overflow:'hidden',boxShadow:'0 4px 24px rgba(0,0,0,0.07)'}}>
          ...
          <div className="pd-info">
            ...
            <h1 className="pd-title" style={{fontWeight:900,color:theme.textColor,margin:'0 0 10px',lineHeight:1.25}}>
              {title}
            </h1>
            ...
            <div style={{display:'flex',alignItems:'baseline',gap:10,marginBottom:16}}>
              ...
              {originalTHB > displayTHB && displayTHB > 0 && (
                <span style={{fontSize:15,color:'#aaa',textDecoration:'line-through'}}>฿{originalTHB.toLocaleString('th-TH')}</span>
              )}
            </div>
            ...
            {hasVariants && (
              <div style={{marginBottom:16}}>
                <label style={{display:'block',fontSize:13,fontWeight:700,color:theme.textColor,marginBottom:6}}>
                  {t('select_option')}:
                </label>
                <select
                  ...
                  style={{width:'100%',padding:'11px 14px',borderRadius:12,border:`2px solid ${selectedVariant ? p : '#e5e7eb'}`,fontSize:14,outline:'none',fontFamily:theme.fontFamily,background:'white',cursor:'pointer'}}
                >
                  <option value="">{tRaw('-- เลือกรูปแบบ --','-- Select option --')}</option>
                  {variants.map((v: any) => {
                    const vTHB = Number(v.price_thb) > 0 ? Number(v.price_thb) : Number(product.price_thb) || 0;
                    return (
                      <option key={v.id} value={v.id}>
                        {v.name}{vTHB > 0 ? ` — ฿${vTHB.toLocaleString('th-TH')}` : ''}
                      </option>
                    );
                  })}
                </select>
