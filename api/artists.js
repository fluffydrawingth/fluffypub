...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (id || slug) {
      ...
      const { data: products } = await supabase.from('products').select('id,title,title_th,title_en,slug,image,cover_image_url,price,price_thb,price_usd,digital_price_thb,digital_price_usd,category,type,is_digital,is_physical,variants').eq('artist_id', data.id).eq('active', true).eq('status','published');
