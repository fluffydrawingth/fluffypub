...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { data: order, error } = await supabase.from('orders').insert({
      user_id: authUser?.id || null, guest_email: authUser ? null : customerEmail,
      customer_name: customerName, customer_email: customerEmail,
      customer_phone: customerPhone || null, shipping_address: shippingAddress || null,
      items: orderItems, subtotal: Math.round(subtotal * 100) / 100, discount, total,
      promo_code: promoCode || null, status: 'pending_payment', payment_status: 'pending',
