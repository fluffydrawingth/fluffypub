...
module.exports = async function handler(req, res) {
  ...
  if (action === 'me') {
    ...
    if (req.method === 'PUT') {
      ...
      if (!user) return;
      const body = req.body || {};
      const fields = ['name','username','bio','avatar_url','first_name','last_name','phone','delivery_email','shipping_address','province','postal_code','preferred_lang','community_about','community_country','community_favorite_medium'];
      const updates = { updated_at: new Date().toISOString() };
      ...
      const PROFILE_SELECT = 'id,email,name,username,role,bio,avatar_url,artist_slug,favorites,first_name,last_name,phone,delivery_email,shipping_address,province,postal_code,preferred_lang,community_about,community_country,community_favorite_medium,social_links';
